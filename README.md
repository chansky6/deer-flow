...
Consulting-analysis workflows also support a structured framework-review checkpoint: the frontend opens the inline review card as soon as framework drafting starts, streams the draft live with both rendered Markdown and raw Markdown views, and the backend now auto-converts the completed framework response into the pending review state without requiring a second explicit review-tool step. That auto-stop also wins over any extra tool calls the model mistakenly emits in the same framework response, so Phase 2 cannot bypass review. The streamed assistant markdown remains the authoritative review draft, normal chat input stays locked only while confirmation is pending, and the confirmed framework card remains visible in read-only mode after submission while that confirmed framework is injected into the next model turn as transient context.
...
- [LangGraph HITL Research](docs/LANGGRAPH_HITL_RESEARCH.md) - Investigation of the project's clarification-based and framework-review human-in-the-loop flows
