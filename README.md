...
Consulting-analysis workflows also support a structured framework-review checkpoint: the frontend now opens the inline review card as soon as framework drafting starts, streams the draft live with both rendered Markdown and raw Markdown views, uses that streamed assistant markdown as the authoritative review draft instead of duplicating the full framework in tool arguments, locks normal chat input only while confirmation is pending, and keeps the confirmed framework card visible in read-only mode after submission while injecting that confirmed framework into the next model turn as transient context.
