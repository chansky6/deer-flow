...
项目里实际落地的是一套**“澄清式 HITL”**：

- 当模型判断用户需求缺失信息、存在歧义或需要确认时，调用 `ask_clarification`。
- 后端中间件拦截这个工具调用。
- 中间件把澄清问题写入消息历史，并直接结束当前轮执行。
- 前端把该工具消息渲染成一个需要用户回答的问题。
- 用户下一次输入时，系统在同一个 thread 上继续执行新的回合。
...
- 后端通过 `ClarificationMiddleware` 拦截并终止当前回合。
- 前端把澄清请求显式展示给用户。
- 用户通过下一条普通消息补充信息后，agent 再继续下一轮推理。
