...
- For `task_type=deep_research`, route can switch to `src/deep_research/workflow.py` (LangGraph plan/retrieve/synthesize/report workflow + quality gate + restricted toolset)

**ThreadState** (`src/agents/thread_state.py`):
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear)

**Runtime Configuration** (via `context` with `configurable` compatibility fallback):
...
- `DEER_FLOW_ENABLE_DEEP_RESEARCH_MIN_FLOW` (env) - DeepResearch minimal stable flow feature flag (`false` to rollback)
